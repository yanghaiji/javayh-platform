package com.javayh.oauth.exception;
...
public class WebException implements WebResponseExceptionTranslator {
	...
	public ResponseEntity<OAuth2Exception> translate(Exception e) throws Exception {
		...
		if (ase != null) {
			return handleOAuth2Exception(
					new WebException.UnauthorizedException(e.getMessage(), e));
		}

		ase = (AccessDeniedException) throwableAnalyzer
				.getFirstThrowableOfType(AccessDeniedException.class, causeChain);
		// if (ase != null) {
		// return handleOAuth2Exception(new
		// WebException.ForbiddenException(ase.getMessage(), ase));
		// }
		//
		...
		// if (ase != null) {
		// return handleOAuth2Exception(new
		// WebException.MethodNotAllowed(ase.getMessage(), ase));
		// }

		return handleOAuth2Exception(new WebException.ServerErrorException(
	...
	private ResponseEntity<OAuth2Exception> handleOAuth2Exception(OAuth2Exception e)
		...
		if (status == HttpStatus.UNAUTHORIZED.value()
			...
			headers.set("WWW-Authenticate", String.format("%s %s",
					OAuth2AccessToken.BEARER_TYPE, e.getSummary()));
		}

		ResponseEntity<OAuth2Exception> response = new ResponseEntity<OAuth2Exception>(e,
