...
public class GenMain {
	...
	public void generate() throws Exception{
		...
		if(!isGenBeanOnly){
			GenMapperJavaFile mapperFile = new GenMapperJavaFile(this, beanFile,beanDtoFile);
			mapperFile.generate();
			GenMapperXmlFile mapperXmlFile = new GenMapperXmlFile(this, beanFile, mapperFile);
			mapperXmlFile.generate();
			GenIServiceFile serviceFile = new GenIServiceFile(this, beanFile,beanDtoFile);
			serviceFile.generate();
			GenServiceImplFile serviceImplFile = new GenServiceImplFile(this, beanFile,beanDtoFile, mapperFile, serviceFile);
			serviceImplFile.generate();
			GenControllerFile genControllerFile = new GenControllerFile(this, beanFile,beanDtoFile, serviceFile);
	...
	private void initConn() throws Exception{
		...
		Connection connection = DriverManager.getConnection(GenerateConfig.jdbcUrl, GenerateConfig.jdbcUsername, GenerateConfig.jdbcPassword);//获取连接
		//查询所有字段
		String sql = "SELECT COLUMN_NAME,COLUMN_COMMENT,DATA_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA " +
				"= '"+GenerateConfig.jdbcName+"' AND TABLE_NAME = '"+tableName+"' order by ORDINAL_POSITION";
		PreparedStatement pst = connection.prepareStatement(sql);//准备执行语句
		ResultSet rSet = pst.executeQuery();
		structureList = convertList(rSet);
		sql = "select TABLE_NAME,TABLE_COMMENT from information_schema.TABLES where TABLE_SCHEMA = '"+GenerateConfig.jdbcName+"' AND TABLE_NAME = '"+tableName+"'";
