...
public class GenMapperJavaFile {
	...
	public void generate(){
		...
		packageName =
				GenerateConfig.BASE_PACKAGE_NAME + "." + GenerateConfig.PROJECT_NAME + "." + gMain.getModelName() +
						"." + GenerateConfig.DAO_NAME;
		className = NameUtils.formatClassName(gMain.getTableName()) + GenerateConfig.DAO_FILENAME;
	...
	private String genContent(){
		...
		content.append("import org.apache.ibatis.annotations.Mapper;").append(lineSeparator);
		...
		content.append("\t").append("List<"+beanFile.getClassName()+">").append(" findByPage( ").append(beanDTOFile.getClassName() + " "+ StringUtil.toFirstUpCase(beanDTOFile.getClassName())).append(")").append(";").append(lineSeparator);
		content.append(lineSeparator);

		//写入单个查找方法
		content.append(lineSeparator);
		content.append(NotesUtils.getMethodNotes("根据id查找"));
		content.append("\t").append(beanFile.getClassName()).append(" findById(@Param(\"").append(gMain.getStructureList().get(0).getFieldName()).append("\") Integer ").append(gMain.getStructureList().get(0).getFieldName()).append(")").append(";").append(lineSeparator);
		...
		content.append("\t").append("int deleteById(@Param(\"").append(gMain.getStructureList().get(0).getFieldName()).append("\") Integer ").append(gMain.getStructureList().get(0).getFieldName()).append(")").append(";").append(lineSeparator);
		content.append("}");
	 ...
	 * @param gMain
	 */
	public GenMapperJavaFile(GenMain gMain,GenBeanFile beanFile,GenBeanDTOFile beanDTOFile) {
		super();
		this.gMain = gMain;
