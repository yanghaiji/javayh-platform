...
public class StringUtil extends StringUtils {
	...
	public static boolean checkWeChatUrl(String url) {
		if (url.length() <= 49) {
			String re1 = "(https)" + "(:)" + "(\\/)" + "(\\/)" // Any Single Character 3
					+ "(mp)" + "(\\.)" // Any Single Character 4
					+ "(weixin)" + "(\\.)" + "(qq)" + "(\\.)" + "(com)" + "(\\/)" + "(s)"
					+ "(\\/)" + "((.*?))";
	...
	public static int indexOf(String baseStr, String[] strings) {
		if (baseStr == null || baseStr.length() == 0 || strings == null
				|| strings.length == 0) {
			return 0;
		}
		int i = 0;
	...
	public static String removeHtml(String news) {
		...
		Pattern pattern = Pattern.compile(
				"<(span)?\\sstyle.*?style>|(span)?\\sstyle=.*?>", Pattern.DOTALL);
		...
		String strnew = matchhttp.replaceAll("")
				.replaceAll("(if[\\s]*\\(|else|elseif[\\s]*\\().*?;", " ");
		...
		String answer = strout.replaceAll("[\\pP‘’“”]", " ").replaceAll("\r", " ")
	...
	public static List<String> arrayToEmpty(String[] array) {
		List<String> lsit = Arrays.stream(array).filter(val -> isNotBlank(val))
	...
	public static Set<String> arrayToSet(String[] array) {
		Set<String> set = Arrays.stream(array).filter(val -> val != null)
	...
	public static boolean isjson(String string) {
		try {
			com.alibaba.fastjson.JSONObject jsonStr = com.alibaba.fastjson.JSONObject
					.parseObject(string);
			return true;
		}
		catch (Exception e) {
	...
	public static float levenshtein(String str1, String str2) {
		...
		// 计算相似度
		float similarity = 1
				- (float) dif[len1][len2] / Math.max(str1.length(), str2.length());
		return similarity;
	}
